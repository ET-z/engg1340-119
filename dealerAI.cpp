...
void dealerDumb(WINDOW *game_win, int &playerHealth) {
    if (rand() % 2 == 1) {
        printCentered(game_win, "[Dumb AI] Dealer shot live!", 7);
        napms(2000);  
        playerHealth = std::max(playerHealth - 20, 0);
    } else {
        printCentered(game_win, "[Dumb AI] Dealer shot blank.", 7);
        napms(2000);  
    }
}
...
void dealerScopeAware(WINDOW *game_win, int &playerHealth, bool currentShell) {
    if (currentShell) {
        printCentered(game_win, "[Scope-Aware AI] Dealer knows it's live! Attacking.", 7);
        napms(2000);  
        playerHealth = std::max(playerHealth - 20, 0);
    } else {
        printCentered(game_win, "[Scope-Aware AI] Dealer knows it's blank. Skipping.", 7);
        napms(2000);  
    }
}
...
void dealerRiskAware(WINDOW *game_win, int &playerHealth, int dealerHealth, int liveCount, int totalShells) {
    ...
    if (dealerHealth < 20 && risk > 0.6) {
        printCentered(game_win, "[Risk-Aware AI] Dealer too weak, skipped shooting.", 7);
        napms(2000); 
        return;
    }
    if (risk > 0.4 || rand() % 2 == 1) {
        printCentered(game_win, "[Risk-Aware AI] Dealer decided to shoot.", 7);
        napms(2000); 
        playerHealth = std::max(playerHealth - 20, 0);
    } else {
...
void dealerSmart(WINDOW *game_win, int &playerHealth, int dealerHealth, bool currentShell, int liveCount, int totalShells) {
    ...
    if (playerHealth <= 20 && (currentShell || risk > 0.5)) {
        printCentered(game_win, "[Smart AI] Player weak, Dealer tries to finish!", 7);
        napms(2000); 
        playerHealth = std::max(playerHealth - 20, 0);
        return;
    }
    ...
    if (dealerHealth <= 20 && risk > 0.5) {
        printCentered(game_win, "[Smart AI] Dealer avoids risky shooting.", 7);
        napms(2000); 
        return;
    }

    if (currentShell || risk > 0.5 || rand() % 2 == 1) {
        printCentered(game_win, "[Smart AI] Dealer attacked smartly.", 7);
        napms(2000); 
        playerHealth = std::max(playerHealth - 20, 0);
    } else {
        printCentered(game_win, "[Smart AI] Dealer skipped.", 7);
        napms(2000); 
    };
}
